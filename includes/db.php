 ...
 */
function select($field_value, $table, $query_field1, $query1)
{
    try {
        include_once "config.php";
        ...
        $statement = $db -> prepare(
            "SELECT $field_value FROM $table WHERE $query_field1 = :q1"
        );

        $statement -> execute(array('q1' => $query1));
...
function select_two($field_value, $table, $query_field1, $query1, $query_field2, $query2)
{
    try {
        include_once "config.php";
        ...
        $statement = $db -> prepare(
            "SELECT $field_value FROM $table WHERE $query_field1 = :q1 AND $query_field2 = :q2"
        );

        $statement -> execute(array('q1' => $query1, 'q2' => $query2));
